use crate::server::service_account_manager::ServiceAccountManager;
use crate::server::service_mount_manager::ServiceMountManager;
use crate::server::{MountDefinition, Shared};
...
pub(super) struct CreateMount {
    ...
    mount_manager: Arc<dyn ServiceMountManager>,
    account_manager: Arc<dyn ServiceAccountManager>,
}
...
impl CreateMount {
    ...
    pub fn create(
        ...
        self.log
            .info(&format!("Creating mounts for {service_name}",));
        todo!();
        // self.token.perform_if_valid(token, move || {
        //     let shares = HashSet::from_iter(mounts.iter().flat_map(|mount| match mount.shared {
        //         Shared::No => None,
        //         Shared::WithServices(service_names) => Some(Share {
        //             name: mount.name.clone(),
        //             guest_service_names: service_names.clone(),
        //         }),
        //     }));

        //     let credentails = or_log_and_return_response_error!(
        //         self.log => warn,
        //         self.account_manager.get_or_create_service_account(&service_name, &shares)
        //     );

        //     let active_path = or_log_and_return_response_error!(
        //         self.log => warn,
        //         self.mount_manager.get_or_create(&service_name, &mounts)
        //     );
