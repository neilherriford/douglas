...
use reconcile_trigger_types::{Request, Response};
...
pub trait Client: Send + Sync {
    async fn notify_pushed(&self, name: &str) -> Result<Response, Error>;
}
...
impl UdsClient {
    pub fn new(reporter: Arc<dyn Reporter>, douglas_folders: &DouglasFolders) -> Self {
        Self {
            reporter,
            socket_path: douglas_folders.socket_file(reconcile_trigger_types::SOCKET_NAME),
        }
    }
...
impl Client for UdsClient {
    async fn notify_pushed(&self, name: &str) -> Result<Response, Error> {
        let guard = Span::new(
            ...
            &format!("Notifying bract that '{name}' was pushed"),
            ScopeKind::Task,
        )
        .start_guard();
