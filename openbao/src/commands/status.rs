...
impl<'a> StatusCommand<'a> {
    pub fn new(rest_client: &'a mut dyn RestClient, parser: &'a JsonParser) -> Self {
        Self {
            rest_client,
            parser,
        }
    ...
    pub async fn perform(&mut self) -> Result<Status, OpenBaoError> {
        ...
        let body = self.get_body(req).await?;
    ...
    async fn get_body(&mut self, req: Request) -> Result<String, OpenBaoError> {
        let body = match self.rest_client.execute(&req).await? {
