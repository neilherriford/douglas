use crate::{Event, EventKind, Level, Outcome, Reporter, ScopeId};
use crossterm::{
            ...
            EventKind::PlanHint { steps } => {
                if steps.is_empty() {
                    ...
                    for (i, step) in steps.iter().enumerate() {
                        self.push_line(format!("  {}. {step}", i + 1));
                    }
                }
            }
            EventKind::Progress { .. } => {}
...
fn render_loop(mut terminal: Terminal<CrosstermBackend<Stdout>>, rx: &mpsc::Receiver<Event>) {
    ...
    // Drain any events that were buffered before the TUI started (e.g. the Enter
    // keypress used to launch the command).
